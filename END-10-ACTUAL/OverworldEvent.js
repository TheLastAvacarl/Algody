class OverworldEvent {
  ...
  handleHome(resolve) {
    ...
    window.location.href = "home.html"; // Example: navigate to home page
  }
  ...
  checkSorted(resolve) {
    ...
    const checkNextItem = () => {
      if (index >= sortedByPosition.length) {
        this.changeCanvasBorderAndPlaySound(hasError ? 'failed' : 'success', resolve);
        return;
      }
      ...
      if (itemInMap && itemInMap.value === sortedValues[index]) {
        ...
        // Set green shadow
        itemInMap.shadow = "/images/characters/green-box-shadow.png"; // Set shadow to green
        console.log(itemInMap.shadow)
      } else {
        console.log(`Item at position ${itemAtPosition.x}, ${itemAtPosition.y} is out of order.`);
        hasError = true;
        // Set red shadow
        itemInMap.shadow = "/images/characters/red-box-shadow.png"; // Set shadow to red
        console.log(itemInMap.shadow);

      }
  
      index++;
      setTimeout(checkNextItem, 500);
    };
